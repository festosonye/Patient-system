...
def generate_pdf_view(request, booking_id):
    # Retrieve the booking instance from the database
    booking = BookingPatient.objects.get(id=booking_id)
    ...
    p.drawString(100, 780, f"Patient: {booking.patient}")
    p.drawString(100, 760, f"State: {booking.get_state_display()}")
    p.drawString(100, 740, f"Lists: {booking.lists}")
    p.drawString(100, 720, f"Created At: {booking.created_at}")
    p.drawString(100, 700, f"Appointment Date: {booking.appointmentDate}")
...
def generate_appointment_report(request):
    bookings = BookingPatient.objects.all()
    return render(request, 'patientApp/booking_report.html', {'bookings': bookings})
