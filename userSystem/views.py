...
def patient_list(request):
    # Fetch patients with related user information and apply initial filtering if search query exists
    search_query = request.GET.get('search')
    patients = Patient.objects.select_related('userID').values('id', 'patient_uhid', 'userID__username')
    if search_query:
        patients = patients.filter(patient_uhid__icontains=search_query) | \
                   patients.filter(userID__username__icontains=search_query)
    
    context = {
        'patients': patients,
    }
    return render(request, 'hospitalApp/patient_list.html', context)



def generate_pdf_view(request, users_id):
    # Retrieve the HospitalAdmin instance from the database
    user = get_object_or_404(HospitalAdmin, id=users_id)
    
    # Prepare the response as a PDF
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="booking_patient_report_{users_id}.pdf"'
    ...
    data = [
        ...
        ["User Type", user.userID.user_Type],
        ["User Type", user.userID.last_name],
        ["User Type", user.userID.email],
        ["User Type", user.userID.first_name],
        ["User Type", user.userID.phone_number],
...
def generate_user_report(request):
    users = HospitalAdmin.objects.all()
    return render(request, 'hospitalApp/userreport.html', {'users': users})

#doctors
def generate_doctor_pdf_view(request, users_id):
    # Retrieve the HospitalAdmin instance from the database
    user = get_object_or_404(Doctor, id=users_id)
    
    # Prepare the response as a PDF
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="Doctor_list_report_{users_id}.pdf"'
    ...
    data = [
        ...
        ["First Name", user.userID.first_name],
        ["Date Joined", user.userID.date_joined],
        ["Email", user.userID.email],
        ["Phone Number", user.userID.phone_number],
        ["Last Login", user.userID.last_login],
        # Add more rows as needed
    ]
...
def doctor_list(request):
    users = User.objects.all()  # Assuming you have an is_doctor field
    context = {
        'users': users,
    }
    return render(request, 'hospitalApp/doctor_list.html', context)


def generate_doctor_report(request):
    users = Doctor.objects.all()
    return render(request, 'hospitalApp/doctorslist.html', {'users': users})
